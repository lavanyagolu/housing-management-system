...
if (isset($_POST['reg_seller'])) {
  ...
  // Finally, register user if there are no errors in the form
  if (count($errors) == 0) { array_push($errors, "test");
  	$password = md5($password_1);//encrypt the password before saving in the database
  	$query = "INSERT INTO user (first_name, last_name, email, ph_no, address,user_name,password,user_type) 
...
if (isset($_POST['add_agent'])) {
  ...
  // by adding (array_push()) corresponding error unto $errors array
   if (empty($emp_id)) { array_push($errors, "Employee id is required"); }
    ...
    if (empty($admin)) { array_push($errors, "Please specify if it is a admin by yes or no"); }
    if (empty($agent)) { array_push($errors, "Please specify if it is a agent by yes or no"); }
...
if (isset($_POST['post_ad'])) {
  ...
  // form validation: ensure that the form is correctly filled ...
  // by adding (array_push()) corresponding error unto $errors array
    if (empty($house_description)) { array_push($errors, "house_description is required"); }
	if (empty($house_id)) { array_push($errors, "house_id is required"); }
    if (empty($address)) { array_push($errors, "address is required"); }
    if (empty($city)) { array_push($errors, "state is required"); }
    if (empty($state)) { array_push($errors, "Phone number is required"); }
	if (empty($zipcode)) { array_push($errors, "zipcode is required"); }   
	if (empty($bedrooms)) { array_push($errors, "number of bedrooms is required"); }
    if (empty($bathrooms)) { array_push($errors, " number of bathrooms is required"); }
    if (empty($price)) { array_push($errors, "Estimated price is required"); }
    if (empty($sqft)) { array_push($errors, "square feet is required"); }
  	...
  	$sql="INSERT INTO house(house_id,house_description, address,city,state,zipcode,bedrooms,bathrooms,price,sqft) 
	     VALUES ('$house_id','$house_description','$address','$city','$state','$zipcode','$bedrooms','$bathrooms','$price','$sqft')";
	$results =  mysqli_query($db, $sql);
	if($results)
     {echo "Success";}
      else{echo "Error";}
	  //$_SESSION['username'] = $username;
  	  //$_SESSION['success'] = "You move to  successfully; please login";
      header('location: sellerad1.php');
...
if (isset($_POST['add_agents'])) {
  ...
  // form validation: ensure that the form is correctly filled ...
  // by adding (array_push()) corresponding error unto $errors array
   if (empty($emp_id)) { array_push($errors, "FirstName is required"); }
    if (empty($employee_name)) { array_push($errors, "LastName is required"); }
	 if (empty($email_id)) { array_push($errors, "Email is required"); }
	 if (empty($password)) { array_push($errors, "Phone number is required"); }
	if (empty($password1)) { array_push($errors, "Address is required"); }  
